...
events <- as.Date(events, "%a %b-%e in %Y")
```
...
library(stringr)
menu <- str_split(menu, " \\.+ ") # split menu by the row of periods separated by spaces

# wanted to extract all names that come after a line break, but was unable to figure out how to
burrito = data.frame(price = str_extract_all(menu, "[0-9].[0-9][0-9]")) # extract all prices for the price

# emf - if you are working with listed data, how might this change your strategy?
...
div = xml_find_all(page, "//tr")
year = xml_find_all(div, ".//td")

# emf - would be good to leave pseudo code in cases where you did not finish so that we can understand your logic and where you hope to go
```
